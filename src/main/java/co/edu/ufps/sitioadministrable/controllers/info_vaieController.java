...
public class info_vaieController extends HttpServlet {
    ...
    protected void guardarArchivo(HttpServletRequest request, HttpServletResponse response)
        ...
        archivoDTO[] vector = new archivoDTO[cant];
        ...
        Facade facade = new Facade();
        archivoDTO archivos[]=null;
        String id_borrarT[] = request.getParameterValues("id_borrarT");
        int cantB = Integer.parseInt(request.getParameter("cantB"));
        System.out.println(cantB);
        PrintWriter out2 = response.getWriter();
        try {
            if(cantB>0){
                archivos = new archivoDTO[id_borrarT.length];
            for (int i = 0; i < id_borrarT.length; i++) {
                archivoDTO a = new archivoDTO();
                a.setId(Integer.parseInt(id_borrarT[i]));
                archivos[i] = a;
            }
            facade.eliminarArchivoById(archivos);
            }
            for (int i = 0; i < cant; i++) {
                    ...
                    if (!file.getSubmittedFileName().equals("")) {
                        ...
                        out = new FileOutputStream(new File(ruta2 + File.separator
                                + nombreT[i].split("\\.")[0] + "." + ext));
                        nuevaRuta = "/sitioAdministrable/ArchivosInfo/" + nombreT[i].split("\\.")[0] + "." + ext;
                        System.out.println("nuevaRuta" + nuevaRuta);
                        filecontent = file.getInputStream();
                        int read = 0;
                        final byte[] bytes = new byte[1024];
            ...
            out2.write("" + facade.registrarInfoVaie(descripcion,vector, "", id,urlR, cant, cantB, archivos));
    ...
    public void consultarArchivos(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        try {
            response.setContentType("text/html;charset=UTF-8");
            Facade f = new Facade();
            if (request.getParameter("id") != null) {
                ArrayList<archivoDTO> list = f.consultarArchivos(Integer.parseInt(request.getParameter("id")));
                Gson g = new Gson();
                PrintWriter out = response.getWriter();
                out.write(g.toJson(list));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void consultarInfoVaie2(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        info_vaieDTO dto;
        Facade f = new Facade();
        ...
        try {
            if (request.getParameter("id") != null) {
                dto = f.consultarInfoVaie2(Integer.parseInt(request.getParameter("id")));
                Gson g = new Gson();
                String rel = g.toJson(dto);
                out.write(rel);
            }
    ...
    public void eliminarMenu(HttpServletRequest request, HttpServletResponse response)
        ...
        PrintWriter out = response.getWriter();
        try {
            System.out.println("eliminar 1: id-" + request.getParameter("id"));
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("consultarMenu") != null) {
            ...
            consultarInfoVaie2(request, response);
        ...
        } else if (request.getParameter("consultarArchivos") != null) {
