...
public class Facade {
    ...
    public boolean actualizarDinamico(String descripcion, String archivo, String nombre, int id, String url)
        ...
        dto.getMenu().setRedirigir(url);
        info_vaieDAO dao = new info_vaieDAO();
        int e = dao.registrarDinamico(dto);
        boolean v = e > 0;
        if (!v) {
    ...
    public boolean registrarInfoVaie(String descripcion, archivoDTO[] archivos, String nombre, int id_menu, String url, int cant, int cantB, archivoDTO[] a) throws Exception {
        info_vaieDTO info = new info_vaieDTO();
        info.setDescripcion(descripcion);
        info.setNombre(nombre);//pendiente
        info.getMenu().setId(id_menu);
        info.setArchivo("");
        info.getMenu().setRedirigir(url);
        info_vaieDAO dao = new info_vaieDAO();
        int e = dao.registrarDinamico(info);
        boolean v = e > 0;
        if (!v) {
            v = dao.actualizarDinamico(info);
            e=dao.buscarIdInfo(id_menu);
        }
        dao.actualizarUrlMenu(info.getMenu());
        if(cantB > 0){
    ...
    public ArrayList<menuDTO> consultarMenu() throws Exception {
        info_vaieDAO dao = new info_vaieDAO();
        ArrayList<menuDTO> menus = dao.consultarMenu();
        for (int i = 0; i < menus.size(); i++) {
            if(menus.get(i).getRedirigir().isEmpty()){
                menus.get(i).setRedirigir("none");
            }
            System.out.println( menus.get(i).getRedirigir());
        }
        return menus;
    }
    ...
    public info_vaieDTO consultarInfoVaie2(int id) throws Exception {
        info_vaieDAO dao = new info_vaieDAO();
        return dao.consultarInfoVaie2(id);
    }

    public ArrayList<archivoDTO> consultarArchivos(int id) throws Exception {
        info_vaieDAO dao = new info_vaieDAO();
        return dao.consultarArchivosInfo(id);
    }

    //ELIMINAR
    public boolean eliminarArchivoById(archivoDTO[] archivos) throws Exception {
        info_vaieDAO dao = new info_vaieDAO();
        return dao.eliminarArchivoById(archivos);
    }
